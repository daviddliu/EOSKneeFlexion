#!/usr/bin/env Rscript
# ANALYSIS 18: PCA-based adjustment for ΔLL ~ preop PT (S1PT), excluding KF from PCA.
# Mirrors analysis5 (KF primary + confounders in PCA) but primary predictor is LATpre_S1PT;
# PCA is fit on PI, lordosis, kyphosis, SVA, T4–L1 PA, age — not PT, not knee flexion.
...
# Complete cases: outcome, primary predictor (S1PT), and all variables that enter PCA
# KF is intentionally omitted from PCA and from required complete-data fields
if (!is.null(age_var)) {
  df_clean <- df %>%
    filter(!is.na(LATpre_S1PT) & !is.na(change_lordosis) & !is.na(LATpre_S1PI) &
           ...
           !is.na(LATpre_SVA_C2_S1) & !is.na(LATpre_T4_L1_PA) &
           !is.na(.data[[age_var]]))
} else {
  df_clean <- df %>%
    filter(!is.na(LATpre_S1PT) & !is.na(change_lordosis) & !is.na(LATpre_S1PI) &
...
cat(paste("\n=== PCA-Based Confounder Analysis: preop PT (KF excluded from PCA) ===\n"))
...
# PCA inputs: same global alignment bundle as analysis5 minus S1PT (primary) and minus KF
confounder_vars <- c(
  "LATpre_S1PI",
  "LATpre_L1_S1",
  "LATpre_L4_S1",
...
cat("\n")
cat("NOTE: Preop S1PT is the primary predictor (not in PCA). Knee flexion is not in PCA.\n\n")
...
ic_results <- data.frame(
  ...
  PT_P = numeric(min(7, length(cumulative_variance)))
)
...
for (n_comp in 1:min(7, length(cumulative_variance))) {
  ...
  ic_results$PT_P[n_comp] <- summary(model_temp)$coefficients[2, 4]
}
...
# Also note when S1PT is not significant at p > 0.05 for each PC count
components_with_p_gt_05 <- which(ic_results$PT_P > 0.05)
if (length(components_with_p_gt_05) > 0) {
  cat("PC counts where preop S1PT is non-significant (p > 0.05):\n")
  for (n_comp in components_with_p_gt_05) {
    cat(sprintf("  %d components: p = %.4f\n", n_comp, ic_results$PT_P[n_comp]))
  }
  cat("\n")
}
...
cat(sprintf("  - S1PT p-value = %.4f", ic_results$PT_P[n_components_use]))
if (ic_results$PT_P[n_components_use] > 0.05) {
...
# Model 1: Simple regression (preop S1PT only)
cat("Model 1: Simple Regression (Preop S1PT Only)\n")
model1 <- lm(change_lordosis ~ LATpre_S1PT, data = df_pca)
summary1 <- summary(model1)
cat(sprintf("  S1PT coefficient: %.4f (p = %.4e)\n", 
            summary1$coefficients[2, 1], summary1$coefficients[2, 4]))
cat(sprintf("  R²: %.4f\n", summary1$r.squared))
cat(sprintf("  Adjusted R²: %.4f\n\n", summary1$adj.r.squared))

# Model 2: Multiple regression with original confounders (no KF; S1PT explicit)
cat("Model 2: Multiple Regression (S1PT + radiographic confounders, no KF)\n")
if (!is.null(age_var)) {
  ...
  model2 <- lm(change_lordosis ~ LATpre_S1PT + LATpre_S1PI + LATpre_L1_S1 +
               LATpre_L4_S1 + LATpre_T2_T12 + LATpre_SVA_C2_S1 + LATpre_T4_L1_PA,
               data = df_pca)
}
...
n_pred_m2 <- if (!is.null(age_var)) 8L else 7L
comparison_df <- data.frame(
  Model = c("Simple (S1PT only)",
            "Multiple (S1PT + confounders, no KF)",
            paste0("Multiple (S1PT + ", n_components_use, " PCs)")),
  S1PT_Coef = c(
  ...
  N_Predictors = c(1L, n_pred_m2, 1L + n_components_use)
)
...
p1 <- ggplot(scree_df, aes(x = Component, y = Eigenvalue)) +
  ...
  labs(
    ...
    y = "Eigenvalue",
    title = "Analysis 18: Scree Plot (PCA of confounders, no PT/KF)"
  ) +
  theme_minimal() +
  theme(
...
p2 <- ggplot(variance_df, aes(x = Component)) +
  ...
  labs(
    ...
    title = "Analysis 18: PCA Variance Explained (confounders without PT/KF)"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
...
p3 <- ggplot(loadings_long, aes(x = Component, y = Variable, fill = Loading)) +
  ...
  labs(
    x = "Principal Component",
    y = "Original Variable",
    title = "Analysis 18: PCA Loadings Heatmap",
    fill = "Loading"
  ) +
  theme_minimal() +
  theme(
...
p4 <- ggplot(coef_comparison, aes(x = Model, y = Coefficient)) +
  ...
  labs(
    x = "Model",
    y = "Preop S1PT coefficient (ΔL1–S1 lordosis)",
    title = "Analysis 18: S1PT coefficient across models (KF not in PCA)"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
...
pca_formula_pc_only <- as.formula(paste("change_lordosis ~", 
                                      paste(paste0("PC", 1:n_components_use), collapse = " + ")))
covariates_model_pca <- lm(pca_formula_pc_only, data = df_pca)
...
# Calculate partial correlation (matches model3 S1PT t-test)
...
r2_pc_only <- summary(covariates_model_pca)$r.squared
...
p5 <- ggplot(df_pca, aes(x = LATpre_S1PT, y = resid_from_pca_covariates)) +
  ...
  labs(
    ...
    y = paste0("Residual ΔL1–S1 after removing PC1–PC", n_components_use, "\n(KF not in PC space)"),
    title = paste0("Analysis 18: S1PT vs ΔLL residual (after ", n_components_use, " PCs)")
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold"),
...
cat("Analysis 18: PCA adjustment for ΔLL ~ preop S1PT (KF excluded from PC space)\n\n")

cat("1. PCA reduced", length(confounder_vars), "correlated confounders to", n_components_use, "orthogonal components\n")
cat(sprintf("   - Explained variance: %.2f%%\n", cumulative_variance[n_components_use] * 100))
cat("   - PCs are orthogonal to each other; preop S1PT is outside the PCA space and may correlate with PCs (see VIF).\n\n")

cat("2. Preop S1PT (primary predictor, not in PCA):\n")
p1s <- summary1$coefficients[2, 4]
p2s <- summary2$coefficients[2, 4]
p3s <- summary3$coefficients[2, 4]
cat(sprintf("   - Simple (S1PT only): coef = %.4f, p = %.4e\n",
            summary1$coefficients[2, 1], p1s))
cat(sprintf("   - + original radiographic confounders (no KF): coef = %.4f, p = %.4e\n",
            summary2$coefficients[2, 1], p2s))
cat(sprintf("   - + PCA bundle (same confounders): coef = %.4f, p = %.4e\n",
            summary3$coefficients[2, 1], p3s))
if (p3s <= 0.05) {
  cat("\n   Residual association: S1PT remains associated with ΔL1–S1 after the PC adjustment\n")
  cat("   (KF was not partialled out via PCA; interpret together with KF-inclusive models). \n")
} else {
  cat("\n   After PC adjustment, S1PT is not significant at p < 0.05.\n")
}
cat("\n")
