...
if (abs(change_percent) > 10 && pval2_kf > 0.05) {
  ...
  cat("  - After adjusting for covariates, knee flexion does not independently predict change in PI-LL mismatch\n")
  ...
  cat("  - Knee flexion has an independent effect on change in PI-LL mismatch\n")
...
cat(sprintf("  Knee Flexion ~ Change in PI-LL Mismatch: r = %.4f\n", r_kf_lordosis))
cat(sprintf("  PI-LL Mismatch ~ Change in PI-LL Mismatch: r = %.4f\n", r_pill_lordosis))
...
cat(sprintf("  Preop Thoracic Kyphosis ~ Change in PI-LL Mismatch: r = %.4f\n", r_t2t12_lordosis))
cat(sprintf("  Preop S1PT ~ Change in PI-LL Mismatch: r = %.4f\n", r_s1pt_lordosis))
cat(sprintf("  Preop SVA ~ Change in PI-LL Mismatch: r = %.4f\n", r_sva_lordosis))
cat(sprintf("  Preop T4-L1 PA ~ Change in PI-LL Mismatch: r = %.4f\n", r_t4l1pa_lordosis))

# Partial correlation: r(knee flexion, change in PI-LL mismatch | all other variables)
...
cat(sprintf("\nPartial correlation (Knee Flexion ~ Change in PI-LL Mismatch, controlling for all covariates):\n"))
