...
if ("PI" %in% names(df)) {
  ...
  stop("analysis5_PCA_confounder_analysis_SVA.R: PI or LATpre_S1PI required for planned L1-S1.")
}
...
# Confounder set: S1PI (preop), Planned L1-S1 (PI − planned PI–LL), L4-S1, Thoracic Kyphosis, S1PT, T4-L1 PA, Age
# Note: SVA is excluded since it's now the outcome variable
if (!is.null(age_var)) {
  df_clean <- df %>%
    filter(!is.na(LATpre_LL_KneeAngle) & !is.na(change_SVA) & !is.na(LATpre_S1PI) &
           ...
           !is.na(LATpre_S1PT) & !is.na(LATpre_T4_L1_PA) & 
           !is.na(.data[[age_var]]))
} else {
  df_clean <- df %>%
    filter(!is.na(LATpre_LL_KneeAngle) & !is.na(change_SVA) & !is.na(LATpre_S1PI) &
...
model1 <- lm(change_SVA ~ LATpre_LL_KneeAngle, data = df_pca)
...
if (!is.null(age_var)) {
  formula_str <- paste("change_SVA ~ LATpre_LL_KneeAngle + LATpre_S1PI + planned_L1_S1 +",
                       "LATpre_L4_S1 + LATpre_T2_T12 + LATpre_S1PT +",
                       "LATpre_T4_L1_PA +", age_var)
  model2 <- lm(as.formula(formula_str), data = df_pca)
} else {
  model2 <- lm(change_SVA ~ LATpre_LL_KneeAngle + LATpre_S1PI + planned_L1_S1 + 
               LATpre_L4_S1 + LATpre_T2_T12 + LATpre_S1PT + LATpre_T4_L1_PA, 
               data = df_pca)
}
...
# Plot 1: Scree plot
png("planned_results/analysis5_scree_plot.png", width = 10, height = 6, units = "in", res = 300)
scree_df <- data.frame(
...
png("planned_results/analysis5_variance_explained.png", width = 10, height = 6, units = "in", res = 300)
...
png("planned_results/analysis5_loadings_heatmap.png", width = 10, height = 8, units = "in", res = 300)
...
png("planned_results/analysis5_model_comparison.png", width = 10, height = 6, units = "in", res = 300)
...
# Create the plot
png("planned_results/analysis5_residual_plot.png", width = 10, height = 8, units = "in", res = 300)
...
if (summary3$coefficients[2, 4] > 0.05) {
  ...
  cat("     the relationship between knee flexion and change in sva (cm).\n")
}
cat("\n")
