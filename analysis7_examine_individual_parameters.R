...
# Base variables: knee flexion and change in lordosis
...
cat(paste("Base sample size:", nrow(df_base), "patients with knee flexion and change in lordosis data\n\n"))
...
model_base <- lm(change_lordosis ~ LATpre_LL_KneeAngle, data = df_base)
...
for (i in seq_along(parameters)) {
  ...
  model_base_subset <- lm(change_lordosis ~ LATpre_LL_KneeAngle, data = df_param)
  ...
  formula_with_param <- as.formula(paste("change_lordosis ~ LATpre_LL_KneeAngle +", param_var))
...
p4 <- ggplot(results_df, aes(x = reorder(Parameter, abs(Partial_R)), y = Partial_R)) +
  ...
  labs(
    ...
    title = "Partial Correlations\n(Relationship with change in lordosis, controlling for knee flexion)"
  ) +
  theme_minimal() +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold", size = 14),
...
all_params_vars <- c("LATpre_LL_KneeAngle", "change_lordosis", tested_params)
...
if (nrow(df_all_params) >= 10) {
  ...
  # Unadjusted model
  model_unadj_all <- lm(change_lordosis ~ LATpre_LL_KneeAngle, data = df_all_params)
  ...
  if (length(tested_params) > 0) {
    formula_adj_all <- as.formula(paste("change_lordosis ~ LATpre_LL_KneeAngle +", 
...
cat("   change in lordosis is explained by its correlation with other preop parameters\n")
cat("   that are the true predictors of change in lordosis.\n\n")
