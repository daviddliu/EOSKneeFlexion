...
cat("  Y (Outcome): Change in Lumbar Lordosis (LAT6W_L1_S1 - LATpre_L1_S1)\n\n")
...
formula_adj <- as.formula(paste("change_lordosis ~ LATpre_LL_KneeAngle +", 
...
for (i in seq_along(blocks)) {
  ...
  if (length(cumulative_vars) > 0) {
    formula_block <- as.formula(paste("change_lordosis ~ LATpre_LL_KneeAngle +", 
...
for (i in seq_along(adjustment_set)) {
  ...
  if (length(vars_without) > 0) {
    formula_drop <- as.formula(paste("change_lordosis ~ LATpre_LL_KneeAngle +", 
                                     paste(vars_without, collapse = " + ")))
  } else {
    formula_drop <- as.formula("change_lordosis ~ LATpre_LL_KneeAngle")
  }
...
for (i in seq_along(adjustment_set)) {
  ...
  formula_base <- as.formula("change_lordosis ~ LATpre_LL_KneeAngle")
  model_base <- lm(formula_base, data = df_complete)
  r2_base <- summary(model_base)$r.squared
  
  formula_with <- as.formula(paste("change_lordosis ~ LATpre_LL_KneeAngle +", var))
  ...
  for (j in seq_len(n_combinations)) {
    ...
    if (length(other_vars) > 0) {
      ...
      if (n_other > 0) {
        ...
        formula_subset <- as.formula(paste("change_lordosis ~ LATpre_LL_KneeAngle +", 
                                          paste(subset_vars, collapse = " + ")))
        model_subset <- lm(formula_subset, data = df_complete)
        r2_subset <- summary(model_subset)$r.squared
        
        formula_subset_with <- as.formula(paste("change_lordosis ~ LATpre_LL_KneeAngle +", 
...
cat("HYPOTHESIS: Knee flexion's apparent association with change in lordosis\n")
...
if (p_unadj < 0.05 && p_adj >= 0.05 && attenuation > 10) {
  ...
  cat("    The apparent association between knee flexion and change in lordosis\n")
  cat("    is fully explained by other preop parameters. Knee flexion does not\n")
  cat("    have an independent causal effect on change in lordosis.\n\n")
} else if (p_unadj < 0.05 && p_adj < 0.05 && attenuation > 20) {
...
cat("   outcome (change in lordosis), creating a spurious association.\n")
...
cat("   between knee flexion and change in lordosis should get smaller (attenuate).\n")
...
cat("    change in lordosis (before accounting for other factors)\n")
...
cat("    Knee flexion does NOT directly cause change in lordosis.\n\n")
