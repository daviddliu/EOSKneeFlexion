...
if (p_unadj < 0.05 && p_adj >= 0.05 && attenuation > 10) {
  ...
  cat("    The apparent association between knee flexion and change in pi-ll mismatch\n")
  cat("    is fully explained by other preop parameters. Knee flexion does not\n")
  cat("    have an independent causal effect on change in pi-ll mismatch.\n\n")
} else if (p_unadj < 0.05 && p_adj < 0.05 && attenuation > 20) {
...
cat("   outcome (change in pi-ll mismatch), creating a spurious association.\n")
...
cat("   between knee flexion and change in pi-ll mismatch should get smaller (attenuate).\n")
...
cat("    change in pi-ll mismatch (before accounting for other factors)\n")
...
cat("    Knee flexion does NOT directly cause change in pi-ll mismatch.\n\n")
