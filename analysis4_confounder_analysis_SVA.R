...
# Calculate change in SVA (convert from mm to cm by dividing by 10) using 6-week data
...
df_clean <- df %>%
  filter(!is.na(LATpre_LL_KneeAngle) & !is.na(change_SVA) & !is.na(preop_PI_LL) &
...
cat("  - Change in SVA (cm) ((LAT6W_SVA_C2_S1 - LATpre_SVA_C2_S1) / 10, 6-week follow-up)\n")

# ============================================================================
# ANALYSIS 1: Simple regression (preop knee flexion ~ change in SVA)
# ============================================================================
cat("\n=== Model 1: Simple Regression (Preop Knee Flexion Only) ===\n")
model1 <- lm(change_SVA ~ LATpre_LL_KneeAngle, data = df_clean)
...
model2 <- lm(change_SVA ~ LATpre_LL_KneeAngle + preop_PI_LL + LATpre_L1_S1 + 
...
if (abs(change_percent) > 10 && pval2_kf > 0.05) {
  ...
  cat("  - After adjusting for covariates, knee flexion does not independently predict change in SVA\n")
  ...
  cat("  - Knee flexion has an independent effect on change in SVA\n")
...
r_kf_lordosis <- cor(df_clean$LATpre_LL_KneeAngle, df_clean$change_SVA, use = "complete.obs")
r_pill_lordosis <- cor(df_clean$preop_PI_LL, df_clean$change_SVA, use = "complete.obs")
r_kf_pill <- cor(df_clean$LATpre_LL_KneeAngle, df_clean$preop_PI_LL, use = "complete.obs")

cat(sprintf("Zero-order correlations:\n"))
cat(sprintf("  Knee Flexion ~ Change in SVA (cm): r = %.4f\n", r_kf_lordosis))
cat(sprintf("  PI-LL Mismatch ~ Change in SVA (cm): r = %.4f\n", r_pill_lordosis))
cat(sprintf("  Knee Flexion ~ PI-LL Mismatch: r = %.4f\n", r_kf_pill))

# Calculate correlations with all included variables
r_l1s1_lordosis <- cor(df_clean$LATpre_L1_S1, df_clean$change_SVA, use = "complete.obs")
r_l4s1_lordosis <- cor(df_clean$LATpre_L4_S1, df_clean$change_SVA, use = "complete.obs")
r_t2t12_lordosis <- cor(df_clean$LATpre_T2_T12, df_clean$change_SVA, use = "complete.obs")
r_s1pt_lordosis <- cor(df_clean$LATpre_S1PT, df_clean$change_SVA, use = "complete.obs")
r_sva_lordosis <- cor(df_clean$LATpre_SVA_C2_S1, df_clean$change_SVA, use = "complete.obs")
r_t4l1pa_lordosis <- cor(df_clean$LATpre_T4_L1_PA, df_clean$change_SVA, use = "complete.obs")

cat(sprintf("  Preop Lordosis L1-S1 ~ Change in SVA (cm): r = %.4f\n", r_l1s1_lordosis))
cat(sprintf("  Preop L4-S1 ~ Change in SVA (cm): r = %.4f\n", r_l4s1_lordosis))
cat(sprintf("  Preop Thoracic Kyphosis ~ Change in SVA (cm): r = %.4f\n", r_t2t12_lordosis))
cat(sprintf("  Preop S1PT ~ Change in SVA (cm): r = %.4f\n", r_s1pt_lordosis))
cat(sprintf("  Preop SVA ~ Change in SVA (cm): r = %.4f\n", r_sva_lordosis))
cat(sprintf("  Preop T4-L1 PA ~ Change in SVA (cm): r = %.4f\n", r_t4l1pa_lordosis))

# Partial correlation: r(knee flexion, change in SVA | all other variables)
...
model_no_kf <- lm(change_SVA ~ preop_PI_LL + LATpre_L1_S1 + 
...
cat(sprintf("\nPartial correlation (Knee Flexion ~ Change in SVA (cm), controlling for all covariates):\n"))
...
p1 <- ggplot(df_clean, aes(x = LATpre_LL_KneeAngle, y = change_SVA)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = TRUE, color = "red") +
  labs(
    x = "Preoperative Knee Flexion",
    y = "Change in SVA (cm) (6-week)",
...
ggsave("planned_results/analysis4_SVA_unadjusted.png", plot = p1, width = 8, height = 6, dpi = 300)
cat("\nSaved unadjusted plot to planned_results/analysis4_SVA_unadjusted.png\n")
...
covariates_model <- lm(change_SVA ~ preop_PI_LL + LATpre_L1_S1 + 
                       LATpre_L4_S1 + LATpre_T2_T12 + LATpre_S1PT + LATpre_SVA_C2_S1 + LATpre_T4_L1_PA, data = df_clean)
...
ggsave("planned_results/analysis4_SVA_adjusted.png", plot = p2, width = 8, height = 6, dpi = 300)
cat("Saved adjusted (residual) plot to planned_results/analysis4_SVA_adjusted.png\n")
...
ggsave("planned_results/analysis4_SVA_coefficient_comparison.png", plot = p3, width = 8, height = 6, dpi = 300)
cat("Saved coefficient comparison plot to planned_results/analysis4_SVA_coefficient_comparison.png\n")
